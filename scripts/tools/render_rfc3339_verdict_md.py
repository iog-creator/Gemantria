...
def main():
    if not VERDICT.exists():
        print("no verdict at evidence/exports_rfc3339.verdict.json", file=sys.stderr)
        return 1
    ...
    for name in sorted(files.keys()):
        ...
        lines.append(
            f"| `{name}` | {tick(f.get('exists', False))} | {tick(f.get('has_generated_at', False))} | {tick(f.get('rfc3339_ok', False))} |"
        )
