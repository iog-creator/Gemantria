...
def update_registry(registry: KBDocumentRegistry, phase_files: dict[str, dict[str, Any]], dry_run: bool = False) -> int:
    ...
    for rel_path, metadata in phase_files.items():
        ...
        if existing:
            ...
            if (
                "dominant_importance" not in existing.provenance
                or existing.provenance.get("dominant_importance") != metadata["importance"]
            ):
            ...
            if needs_update:
                ...
                updated += 1
                print(
                    f"[tag_registry] Updated: {rel_path} (phase {metadata['phase_key']}, importance {metadata['importance']})"
                )
        else:
            # Create new document
            doc = KBDocument(
            ...
            created += 1
            print(
                f"[tag_registry] Created: {rel_path} (phase {metadata['phase_key']}, importance {metadata['importance']})"
            )
