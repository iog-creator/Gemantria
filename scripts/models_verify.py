...
if miss:
    print("MISSING_IN_ENV", ",".join(miss))
    sys.exit(2)
...
for k, v in want.items():
    ...
    if (
        re.search(rf"^{k}\s*=\s*{re.escape(v)}\b", manifest, re.M) is None
        and (k != "EMBEDDING_MODEL" or v in manifest)
        and (k != "RERANKER_MODEL" or v in manifest)
    ):
        bad.append((k, v))

if bad:
    for k, v in bad:
        print(f"DRIFT:{k}='{v}' not found in MODEL_MANIFEST")
