...
def main() -> int:
    ...
    html.append(
        "        #manifest-table th, #manifest-table td { border: 1px solid #e1e4e8; padding: 8px; text-align: left; }"
    )
    ...
    html.append("            try{")
    html.append("                const r = await fetch('centrality.json');")
    html.append("                if(!r.ok) return;")
    html.append("                const c = await r.json();")
    html.append("                const n = Object.keys(c).length;")
    html.append("                const top = Object.entries(c)")
    ...
    html.append('                    "current   strong=" + cur.strong + " weak=" + cur.weak,')
    html.append('                    "previous  strong=" + prev.strong + " weak=" + prev.weak,')
    html.append('                    "delta     strong=" + (dd.strong>=0? "+"+dd.strong: dd.strong) +')
    ...
    html.append(
        ...
        '" edges=" + (p.counts?.edges ?? 0) + '
        '" artifacts=" + (p.counts?.artifacts ?? "?"),'
    )
    html.append(
        '                    "delta=" + (p.delta_present ? "yes":"no") + '
        '" centrality=" + (p.centrality_present ? "yes":"no")'
    )
    html.append("                ];")
