...
def percentile(vals, p):
    ...
    vals = sorted(vals); k = (len(vals)-1)*p; f=int(k); c=min(f+1, len(vals)-1); d=k-f
    return vals[f]*(1-d) + vals[c]*d

def main()->int:
    if not GRAPH.exists(): print("[calibrate] missing graph"); return 2
    ...
    print(f"[calibrate] wrote {OUT.relative_to(ROOT)}"); return 0
