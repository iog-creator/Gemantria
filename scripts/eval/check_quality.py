...
def main() -> int:
    ...
    lines.append(
        ...
        f"min_strong_or_weak={min_strong_or_weak} "
        f"require_nonzero_eig={require_nonzero_eig}"
    )

    failed = False
    if miss_pct > max_missing_pct:
        lines.append("FAIL: missing_rerank_pct above threshold")
        failed = True
    if (strong + weak) < min_strong_or_weak:
        lines.append("FAIL: not enough strong/weak edges")
        failed = True
    if require_nonzero_eig and not eig_nonzero:
        lines.append("FAIL: eigenvector centrality all zero")
        failed = True
