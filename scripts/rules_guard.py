...
def main():
    ...
    doc_markers = (
        ...
        "README.md",
        "AGENTS.md",
        "webui/graph/README.md",
        "webui/graph/AGENTS.md",
    )
    ...
    require(file_exists(ov), "Forest overview missing. Run: python scripts/generate_forest.py")
    max_age = 60 * 60 * 24
    ...
    if CI:
        ...
        require(age <= max_age or (allow_old and not CI), "Forest overview older than 24h. Regenerate forest.")
    else:
        require(age <= max_age or allow_old, "Forest overview older than 24h. Regenerate forest or set ALLOW_OLD_FOREST=1 (not allowed on CI).")

    # 4) SSOT schemas present (presence)
    stats_schema = SSOT / "graph-stats.schema.json"
    cor_schema   = SSOT / "graph-correlations.schema.json"
    require(file_exists(stats_schema), "Missing SSOT schema: docs/SSOT/graph-stats.schema.json")
    ...
    exports_changed = any(
        ...
        for f in ["exports/graph_stats.json", "exports/graph_correlations.json", "scripts/export_stats.py"]
    )
    ...
    if CI and exports_changed:
        have_evidence = any(k in PR_BODY.lower() for k in ["graph_stats", "graph_correlations", "verifier_pass"])
        require(have_evidence, "PR body missing exports/correlations evidence (mention graph_stats/graph_correlations or VERIFIER_PASS).")

    # 6) PR template checklist sanity (CI only, best-effort)
    if CI and code_touched:
        # Look for checklist marks like [x] or [X]
        checklist_ok = bool(re.search(r"Docs Updated\s*- \[x\]", PR_BODY, flags=re.IGNORECASE))
