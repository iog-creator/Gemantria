...
try:
    ...
    print("[sample.check] install jsonschema: pip install jsonschema", file=sys.stderr); sys.exit(2)
...
def extract_enrichment_from_logs(log_text: str) -> list[dict]:
    ...
    for line in log_text.splitlines():
        line = line.strip()
        if not line.startswith('{"level": "INFO"'): continue
        try:
            log_entry = json.loads(line)
            if (log_entry.get("logger") == "gemantria.enrichment" and
                log_entry.get("msg") == "noun_enriched"):
...
def main():
    p = latest_sample()
    if not p:
        print("[sample.check] no sample found in logs/", file=sys.stderr); return 2
    ...
    if not rows:
        print(f"[sample.check] no enrichment records found in {p}", file=sys.stderr); return 2

    bad=0; ok=0
    for i,row in enumerate(rows,1):
        try: validate(row, SCHEMA); ok+=1
        except ValidationError as e: print(f"[sample.check] FAIL {p}:{i} → {e.message}"); bad+=1
    if bad: print(f"[sample.check] {bad} invalid / {ok} valid"); return 1
    print(f"[sample.check] OK ({ok} rows)"); return 0
