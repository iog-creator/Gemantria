...
try:
    ...
    print("[exports.smoke] psycopg (v3) is required. pip install \"psycopg[binary]\"", file=sys.stderr)
    sys.exit(2)

DSN = os.getenv("GEMATRIA_DSN") or os.getenv("DB_DSN") or "postgresql://localhost/gemantria"

PROBES = [
    ("concept_network count", "SELECT COUNT(*) FROM concept_network", lambda row: row[0] > 0),
    ("concept_relations count", "SELECT COUNT(*) FROM concept_relations", lambda row: row[0] > 0),
