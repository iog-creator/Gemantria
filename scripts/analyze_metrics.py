...
from src.graph.metrics import cluster_semantic_diversity, density, semantic_cohesion, bridge_score, diversity_local
...
if __name__ == "__main__":
    ...
    by_cluster = {}
    for cid,cl,vec in items: by_cluster.setdefault(cl,[]).append((cid,vec))

    # cluster metrics
    for cl, pairs in by_cluster.items():
        ids = [cid for cid,_ in pairs]; vecs=[v for _,v in pairs]; n=len(ids)
        e_within = list(db.execute("""
    ...
    for cid,cl,v in items:
        ...
        out_ids=[i for i in all_vec.keys() if i not in cluster_ids][:300]
