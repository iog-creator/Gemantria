...
- os.getenv("GEMATRIA_DSN"/aliases) -> get_rw_dsn()
- os.environ.get("GEMATRIA_DSN"/aliases) -> get_rw_dsn()
- ... for RO side -> get_bible_db_dsn()

Also ensures: from scripts.config.env import get_rw_dsn, get_bible_db_dsn
...
def ensure_import(text: str) -> str:
    if "from scripts.config.env import get_rw_dsn, get_bible_db_dsn" in text:
        return text
    ...
    for i, line in enumerate(lines[:50]):
        if line.lstrip().startswith("import ") or line.lstrip().startswith("from "):
            insert_at = i + 1
    lines.insert(insert_at, "from scripts.config.env import get_rw_dsn, get_bible_db_dsn\n")
    return "".join(lines)
...
def process(path: p.Path) -> bool:
    ...
    out = src
    out = replace_env(out, RW_KEYS, "get_rw_dsn()")
    out = replace_env(out, RO_KEYS, "get_bible_db_dsn()")
    if out != src:
