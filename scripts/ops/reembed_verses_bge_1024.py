    ...
    with tqdm(total=total_to_process, desc="Re-embedding") as pbar:
        ...
        while True:
            ...
            try:
                ...
                embeddings = client.get_embeddings(texts, model="text-embedding-bge-m3")

                # Verify dimension
                if embeddings and len(embeddings[0]) != 1024:
                    raise RuntimeError(f"Embedding dimension mismatch: expected 1024, got {len(embeddings[0])}")
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=500, help="Number of verses per batch (default: 500)")
    parser.add_argument("--resume", action="store_true", help="Resume mode: only re-embed NULL embeddings")
    ...
    with psycopg.connect(dsn) as conn:
        ...
        reembed_batch(client=client, conn=conn, batch_size=args.batch_size, resume=args.resume)
