...
try:
    ...
    print("[goldens.enrichment] install jsonschema: pip install jsonschema", file=sys.stderr); sys.exit(2)
...
def main():
    ...
    for i, line in enumerate(GOLDEN.read_text(encoding="utf-8").splitlines(), 1):
        ...
        try:
            validate(json.loads(line), SCHEMA); ok+=1
        except ValidationError as e:
            print(f"[goldens.enrichment] FAIL line {i}: {e.message}"); bad+=1
    if bad:
        print(f"[goldens.enrichment] {bad} invalid / {ok} valid"); return 1
    print(f"[goldens.enrichment] OK ({ok} rows)"); return 0
