...
- Wire pytest collection under `pmagent/tests/extractors/` (unit-only).
- Keep Tool Bus OFF; adapters RO-only on tag lane (no PR CI probes).

## Deliverables
- Tests: `pmagent/tests/extractors/test_extraction_correctness.py` (TV-E01..E05 placeholders).
- Fixtures: `pmagent/tests/extractors/fixtures/*.json` (tiny goldens).
