...
- **Guarded tool calls → agent_run**: The Phase-1 Guard Shim (`pmagent/guarded/guard_shim.py`) and Gatekeeper (`pmagent/guarded/gatekeeper.py`) record each guarded tool call into `control.agent_run`, including PoR status (`por_ok`), JSON Schema status (`schema_ok`), provenance status (`provenance_ok`), violation codes (`MISSING_POR`, `RING_VIOLATION`, etc.), seed/model/tool_version, latency, and retry counts. Materialized views aggregate these rows into 7d/30d compliance ratios and top violation maps, which are exported to `share/atlas/control_plane/*.json`.
...
- **6J**: BibleScholar Gematria adapter (read-only) ✅ **COMPLETE** (2025-11-15)
  - `pmagent/biblescholar/gematria_adapter.py` — Read-only adapter for Gematria numerics
...
- **6O**: Vector similarity adapter + verse-similarity flow ✅ **COMPLETE** (2025-11-15, PR #557)
  - `pmagent/biblescholar/vector_adapter.py` — Vector similarity adapter (pgvector)
  - `pmagent/biblescholar/vector_flow.py` — Verse-similarity flow wrapper
...
- **6P**: BibleScholar Reference Answer Slice ✅ **COMPLETE** (2025-11-21)
  ...
  - **Tests**: `pmagent/tests/atlas/test_phase6p_biblescholar_reference_guard.py` (5 tests, all passing)
...
- **6D**: Downstream app read-only wiring (StoryMaker + BibleScholar) ✅ **COMPLETE** (2025-11-21)
  ...
  - Adapter module: `pmagent/control_widgets/adapter.py` with `load_graph_compliance_widget_props()` and `load_biblescholar_reference_widget_props()`
  ...
  - Tests: `pmagent/tests/control_widgets/test_adapter.py` (6 tests, all passing)
