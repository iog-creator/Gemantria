...
export default function DBTile({ data, status, reason }: TileSummaryProps) {
    ...
    const handleTest = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setTestResult(`Test passed: ${result.message || "DB reachable"}`);
            } else {
        ...
        <div style={{ fontSize: '0.875rem' }}>
            ...
            {mode !== "unknown" && (
                <div style={{ fontSize: '0.75rem', color: '#4b5563', marginBottom: '0.5rem' }}>
                    <p style={{ margin: '0.25rem 0' }}>Mode: {mode}</p>
                    {totalChecks > 0 && (
                        <p style={{ margin: '0.25rem 0' }}>Checks: {passedChecks}/{totalChecks} passed</p>
                    )}
                </div>
            )}
