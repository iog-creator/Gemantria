...
PROM_EXPORTER_ENABLED = os.getenv("PROM_EXPORTER_ENABLED", "0") not in ("0","false","False")
...
def metrics():
    ...
    lines.append("# HELP gemantria_node_items_out_24h Total items_out over the last 24 hours by node.")
    ...
    lines.append("# HELP gemantria_node_latency_ms_7d Latency statistics over the last 7 days by node.")
    lines.append("# TYPE gemantria_node_latency_ms_7d summary")
    for node, calls, avg_ms, p50, p90, p95, p99 in data.get("latency", []):
        lines.append(f'gemantria_node_latency_ms_7d_sum{{node="{node}"}} {avg_ms * max(calls,1)}')
        lines.append(f'gemantria_node_latency_ms_7d_count{{node="{node}"}} {calls}')
        lines.append(f'gemantria_node_latency_ms_7d_quantile{{quantile="0.5",node="{node}"}} {p50}')
        lines.append(f'gemantria_node_latency_ms_7d_quantile{{quantile="0.9",node="{node}"}} {p90}')
        lines.append(f'gemantria_node_latency_ms_7d_quantile{{quantile="0.95",node="{node}"}} {p95}')
        lines.append(f'gemantria_node_latency_ms_7d_quantile{{quantile="0.99",node="{node}"}} {p99}')
