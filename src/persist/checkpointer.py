...
try:
    import psycopg
except Exception:  # defer import errors to runtime path that needs it
    psycopg = None
...
class MemoryCheckpointer(BaseCheckpointer):
    ...
    def start_run(self, book: str) -> str:
        rid = str(uuid.uuid4())
        self._runs[rid] = {"book": book, "events": []}
        return rid
...
def get_checkpointer() -> BaseCheckpointer:
    ...
    if mode == "postgres":
        ...
        if not dsn:
            raise RuntimeError("GEMATRIA_DSN is required when CHECKPOINTER=postgres")
