...
import CorrelationExplorer from '../views/CorrelationExplorerV2';  // Phase 10: Correlation Visualization
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<'stats' | 'graph' | 'temporal' | 'xrefs' | 'correlation'>('correlation');  // Default to Correlation Explorer for review
  ...
  return (
    <div className="App" style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      ...
      {/* Tab Navigation - Correlation Explorer always available (Phase 10) */}
      <div style={{ marginTop: '30px', marginBottom: '20px' }}>
        ...
        <button  // Phase 10: Correlation Explorer (always available)
          onClick={() => setActiveTab('correlation')}
          style={{
            padding: '10px 20px',
            backgroundColor: activeTab === 'correlation' ? '#007bff' : '#f8f9fa',
            color: activeTab === 'correlation' ? 'white' : 'black',
            border: '1px solid #ddd',
            borderRadius: '4px',
            cursor: 'pointer',
          }}
      ...
      {activeTab === 'correlation' && <CorrelationExplorer />}  // Phase 10: Correlation Visualization
    </div>
  );
}
