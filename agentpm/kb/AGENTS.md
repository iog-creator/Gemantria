...
- **owning_subsystem**: Owning subsystem (agentpm, docs, rules, scripts, etc.)
...
The KB registry is integrated into `pm.snapshot` via `agentpm.status.snapshot.get_system_snapshot()`:
...
```python
from agentpm.kb.registry import load_registry, query_registry
...
agentpm_docs = query_registry(registry, owning_subsystem="agentpm")

# Query by tags (document must have all specified tags)
governance_docs = query_registry(registry, tags=["governance", "root"])

# Combined filters
results = query_registry(registry, type="agents_md", owning_subsystem="agentpm", tags=["governance"])
```
...
- **AGENTS.md files**: Root `AGENTS.md`, `agentpm/AGENTS.md`, `webui/graph/AGENTS.md`
...
- `tags`: Tags for categorization
- `owning_subsystem`: Owning subsystem (docs, agentpm, webui, root, etc.)
...
- Both commands use `agentpm.status.snapshot.get_kb_status_view()` helper
...
**Hints Generator**: `agentpm.status.snapshot.get_kb_hints()` — Generates structured hints from KB registry status
