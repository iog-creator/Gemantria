...
xfail_reason = "Staged TV: Atlas screenshots + index/backlink smoke pending implementation"
...
def test_e23_atlas_index_exists_or_link_present():
    ...
    assert link or atlas_index, "no atlas index link or file (staged)"
...
def test_e25_graph_view_link_present():
    ...
    data = json.load(open(export_path, encoding="utf-8"))
    links = ((data.get("meta") or {}).get("links") or {})
    assert "atlas_graph_view" in links, "missing atlas_graph_view (staged)"
