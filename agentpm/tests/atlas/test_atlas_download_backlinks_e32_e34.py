...
xfail_reason = "Staged TV: node JSON download link + jumper back-links + sitemap/counts pending implementation"
...
def test_e32_node_page_has_json_download_link():
    ...
    assert ("id=\"download-json\"" in html) or ("download" in html and ".json" in html) or re.search(r'href="[^"]+\.json"', html), "no JSON download link (staged)"
