...
def test_lm_status_html_handles_db_off() -> None:
    ...
    assert (
        "LM Studio is offline" in content
        or "database connection unavailable" in content.lower()
...
def test_lm_status_html_shows_healthy_state() -> None:
    ...
    assert (
        "successRate >= 0.95" in content
        or "success_rate >= 0.95" in content
        or "LM Studio is healthy" in content
...
def test_lm_status_html_shows_degraded_state() -> None:
    ...
    assert (
        "LM Studio is degraded" in content
        or "error rate" in content.lower()
        or "degraded" in content.lower()
...
def test_lm_status_html_shows_lm_studio_ratio() -> None:
    ...
    content = STATUS_HTML.read_text(encoding="utf-8")
    assert (
        "lm-studio-ratio" in content
        or "lm_studio_usage_ratio" in content
        or "LM Studio Usage" in content
...
def test_lm_status_html_explains_success_rate() -> None:
    ...
    content = STATUS_HTML.read_text(encoding="utf-8")
    assert (
        "Percentage of LM calls" in content
        or "successfully" in content.lower()
        or "95%+ indicates healthy" in content
...
def test_lm_status_html_explains_error_rate() -> None:
    ...
    assert (
        ...
        or "under 5% is normal" in content
        or "above 10% may indicate" in content
