...
from agentpm.runtime import lm_budget
...
def test_check_lm_budget_no_budget_table_is_fail_open(monkeypatch):
    ...
    mock_psycopg = Mock()
    monkeypatch.setattr("agentpm.runtime.lm_budget.psycopg", mock_psycopg)
...
def test_guarded_lm_call_respects_budget(monkeypatch):
    ...
    with patch("agentpm.runtime.lm_logging._write_agent_run", return_value=None):
...
def test_guarded_lm_call_within_budget_uses_lm_studio(monkeypatch):
    ...
    with patch("agentpm.runtime.lm_logging.lm_studio_chat_with_logging", return_value=mock_result):
        result = guarded_lm_call(
            call_site="test.generate_text",
            messages=[{"role": "user", "content": "hello"}],
            app_name="test-app",
