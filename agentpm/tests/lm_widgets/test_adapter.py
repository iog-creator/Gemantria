...
from agentpm.lm_widgets.adapter import (
...
def test_adapter_healthy(tmp_path: Path) -> None:
    ...
    with patch("agentpm.lm_widgets.adapter.INDICATOR_PATH", test_file):
...
def test_adapter_missing_file(tmp_path: Path) -> None:
    ...
    with patch("agentpm.lm_widgets.adapter.INDICATOR_PATH", non_existent_file):
        props = load_lm_indicator_widget_props()
