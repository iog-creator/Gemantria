...
The `agentpm/extractors/` directory contains provenance management functions for extraction agents. These functions ensure that all extraction outputs carry proper metadata (model, seed, timestamp) for traceability and validation, implementing PLAN-072 M2+ Extraction Agents provenance requirements (TVs E06-E10).
...
Tests are located in `agentpm/tests/extractors/test_extraction_provenance_e06_e10.py`:
...
- **Graph Assembly**: Uses provenance for batch correlation and rollup (see `agentpm/graph/assembler.py`)
