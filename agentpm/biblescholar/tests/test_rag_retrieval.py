...
from agentpm.biblescholar.rag_retrieval import RAGRetriever


class TestRAGRetrieval:
    """Test suite for RAG retrieval engine."""

    @patch("agentpm.biblescholar.rag_retrieval.EmbeddingAdapter")
    @patch("agentpm.biblescholar.rag_retrieval.RerankerAdapter")
    def test_tv_phase15_wave2_01_basic_retrieval(self, mock_reranker_cls, mock_embedding_cls):
        ...
        assert result[0]["embedding_score"] == 0.92
    ...
    def test_tv_phase15_wave2_02_reranker_improves_ranking(self, mock_reranker_cls, mock_embedding_cls):
        ...
        def mock_rerank(chunks, query):
            # Add reranker scores that flip the order
            chunks[0]["reranker_score"] = 0.95  # Boost first chunk
            chunks[1]["reranker_score"] = 0.70  # Lower second chunk

            # Compute combined scores
            for chunk in chunks:
                chunk["combined_score"] = 0.7 * chunk["embedding_score"] + 0.3 * chunk["reranker_score"]
        ...
        assert len(result) == 2
        # First result should have higher combined score due to reranking
