...
# from agentpm.biblescholar.contextual_chunks import expand_context_window  # Wave-3
...
class RAGRetriever:
    ...
    def retrieve_contextual_chunks(self, query: str, top_k: int = 5) -> list[dict[str, Any]]:
        ...
        # Step 3: Build initial chunks with embedding scores
        chunks = []
        ...
        # Step 4: Apply reranker fallback
        # Reranker will update chunks with reranker_score and combined_score
        chunks = self.reranker_adapter.rerank_chunks(chunks, query)

        # Update relevance_score to combined_score (if reranker ran)
        for chunk in chunks:
            if "combined_score" in chunk:
                chunk["relevance_score"] = chunk["combined_score"]
