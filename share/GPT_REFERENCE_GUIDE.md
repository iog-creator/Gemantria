...
- **`EMBEDDING_MODEL`** – Vector model (default: `granite-embedding:278m` - Granite stack).
- **`THEOLOGY_MODEL`** – Main reasoning/theology model.
- **`LOCAL_AGENT_MODEL`** – Local agent model (default: `granite4:tiny-h` - Granite stack).
- **`MATH_MODEL`** – Optional math-heavy model.
- **`RERANKER_MODEL`** – Reranker model (default: `granite4:tiny-h` - Granite stack).
- **`RETRIEVAL_PROFILE`** – `DEFAULT` (default) uses Granite stack. `GRANITE` or `BIBLE` switches retrieval embeddings/rerankers accordingly. Granite IDs are resolved via `GRANITE_EMBEDDING_MODEL`, `GRANITE_RERANKER_MODEL`, and `GRANITE_LOCAL_AGENT_MODEL`. Missing Granite overrides trigger a HINT and fall back to DEFAULT.
...
**Phase-7E Profiles:**
- **DEFAULT (Granite)**: Default stack for general retrieval and local agent - Granite embedding + reranker + local agent (see `env_example.txt` for configuration)
- **BIBLE**: Bible/multilingual lane (BGE-based) - reserved for Bible-specific pipelines, not the general default
- **Granite models** are available in both LM Studio and Ollama:
...
- **DEFAULT profile**: Granite stack (default for general retrieval and local agent)
- **BIBLE profile**: Bible/multilingual lane (BGE-based, reserved for Bible-specific tasks)
