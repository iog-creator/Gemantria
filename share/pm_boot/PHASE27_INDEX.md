...
## 27.H — PM Boot Surface

### Problem

Currently:
- PM/OA boot requires hunting across multiple `share/` subdirectories and `docs/SSOT/` to find kernel, bootstrap, contracts, and phase indexes
- No single "boot capsule" that contains all PM/OA-critical surfaces in one place
- New PM/OA sessions must manually locate and read multiple files

### Objectives

1. **PM Boot Capsule:**
   - Create `share/pm_boot/` as a single directory containing all PM/OA boot-critical files
   - Seeds the capsule from kernel, bootstrap, REALITY_GREEN_SUMMARY, phase indexes, SHARE_FOLDER_ANALYSIS, EXECUTION_CONTRACT, GITHUB_WORKFLOW_CONTRACT, ORCHESTRATOR_REALITY, and AGENTS.md
   - Includes `logs/` subdirectory for session logs (retention policy deferred)

2. **Guard Enforcement:**
   - Implement `scripts/guards/guard_pm_boot_surface.py` to validate capsule completeness
   - Wire guard into `make ops.kernel.check` (mandatory preflight)
   - Wire guard into `make reality.green` (system truth gate)

3. **Generator Script:**
   - Create `scripts/pm/generate_pm_boot_surface.py` to populate capsule from canonical sources
   - Deterministic copy operation (no transformations)
   - Can be called by housekeeping in future phases

### Deliverables

- **PM Boot Capsule**: `share/pm_boot/` directory with required files
  - PM_KERNEL.json, PM_BOOTSTRAP_STATE.json, REALITY_GREEN_SUMMARY.json, HANDOFF_KERNEL.json
  - PHASE24_INDEX.md through PHASE27_INDEX.md
  - SHARE_FOLDER_ANALYSIS.md, EXECUTION_CONTRACT.md, GITHUB_WORKFLOW_CONTRACT.md, ORCHESTRATOR_REALITY.md
  - AGENTS.md
  - logs/ subdirectory
- **Guard Script**: `scripts/guards/guard_pm_boot_surface.py`
  - Validates all required files present
  - Treats planning_context.json as optional
  - Returns JSON summary with ok/missing_required/missing_optional
- **Generator Script**: `scripts/pm/generate_pm_boot_surface.py`
  - Copies canonical sources into capsule
  - Idempotent and deterministic
- **Integration**:
  - `make ops.kernel.check` includes `guard.pm_boot.surface`
  - `make reality.green` includes `guard.pm_boot.surface`
- **Documentation**:
  - Updated `SHARE_FOLDER_ANALYSIS.md` with pm_boot capsule description
  - Updated `PHASE27_INDEX.md` with 27.H subsection

### Acceptance Criteria

- ✅ `share/pm_boot/` exists with all required files
- ✅ `make guard.pm_boot.surface` passes
- ✅ `make ops.kernel.check` includes pm_boot guard and passes
- ✅ `make reality.green` includes pm_boot guard and passes
- ✅ Documentation updated in SSOT docs

---

## 27.I — Open Questions for PM Review
