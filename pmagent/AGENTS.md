...
Subdirectories (e.g. `pmagent/db/`, `pmagent/control_plane/`, `pmagent/runtime/`, `pmagent/mcp/`) each have their own `AGENTS.md` for more detailed contracts.
...
- **DSNs & loaders**
  - All database access must go through centralized loaders:
    ...
    - `pmagent.db.loader.get_control_engine()` / `get_bible_engine()`
  ...
  - AI interaction and governance artifacts live in the `gematria` database:
    - `control.agent_run` - Runtime LM calls and tool executions (written by `pmagent.runtime.lm_logging._write_agent_run()`)
    - `control.agent_run_cli` - CLI command executions (written by `pmagent.control_plane.create_agent_run()` / `mark_agent_run_success()` / `mark_agent_run_error()`)
    - `public.ai_interactions` - Legacy/alternative AI tracking (if used)
    - `public.governance_artifacts` - Governance artifact tracking
  - **Writers:**
    - Runtime LM calls: `pmagent.runtime.lm_logging._write_agent_run()` writes to `control.agent_run` for all `lm_studio_chat_with_logging()` and `guarded_lm_call()` invocations
    - CLI commands: `pmagent.control_plane.create_agent_run()` creates `control.agent_run_cli` records; `mark_agent_run_success()` / `mark_agent_run_error()` update them
  ...
  - **Capability Session → AI Tracking Mapping (Design-only, no writes yet):**
    ...
    - Hermetic validator: `pmagent.reality.capability_envelope_validator` validates envelopes against the contract without writing to DB
    - CLI validation: `pmagent reality validate-capability-envelope` and `validate-capability-history` check envelope structure
    - **Writer path (not yet implemented):** Will use `pmagent.control_plane.create_agent_run()` helper behind explicit opt-in flag
- **Control-plane exports**
  - Control-plane helpers in `pmagent.control_plane` read/write Postgres-backed views and exports (e.g. LM indicator, control status, tool catalog).
  - These exports are consumed by Atlas dashboards, pmagent CLI commands, and reality-check flows.

## Key Components (High-Level)

- **`pmagent/db/`**: DSN loaders, SQLAlchemy engines, and Postgres-specific helpers.
- **`pmagent/control_plane/`**: Control schema helpers (tables, sessions, exports, pipeline status).
- **`pmagent/runtime/`**: LM routing, budgeting, and logging into `public.ai_interactions` / `public.governance_artifacts`.
- **`pmagent/lm/` + `pmagent/adapters/`**: Provider routing for Ollama + LM Studio (chat, embeddings, rerankers, theology).
- **`pmagent/status/`**: System and LM status aggregation used by `/api/status/*` and pmagent status CLI.
  - **`pmagent/status/snapshot.py`**: Unified system snapshot helper (M3 + M4)
  - **`pmagent/status/eval_exports.py`**: Eval exports helpers for Phase-8/10 analytics (M4)
- **`pmagent/kb/`**: Knowledge-base document registry (KB-Reg:M1 + M2 + M6).
  - **`pmagent/kb/registry.py`**: KB document registry model (Pydantic), persistence helpers, `query_registry()` planning helper, and `analyze_freshness()` drift detector
  ...
  - **Snapshot integration (KB-Reg:M2)**: Registry summary included in `pm.snapshot` via `pmagent.status.snapshot.get_system_snapshot()` (advisory-only, non-gating)
...
- **`pmagent/plan/`**: Planning workflows powered by KB registry (AgentPM-Next:M1 + M2).
  - **`pmagent/plan/kb.py`**: KB document worklist builder (M1) — `build_kb_doc_worklist()` produces prioritized worklist from KB registry status and hints
  - **`pmagent/plan/fix.py`**: Doc-fix executor (M2) — `build_fix_actions()` and `apply_actions()` consume worklist and execute doc fixes (create stubs, mark stale, sync metadata)
  ...
  - **See**: `pmagent/plan/AGENTS.md` for detailed contracts
  - **`pmagent/plan/kb.py`**: KB document worklist builder (`build_kb_doc_worklist()`) that produces prioritized documentation tasks from KB registry status and hints
...
- **`pmagent/reality/`**: Reality-check orchestrator helpers (`reality.check` verdicts).
- **`pmagent/atlas/` + `pmagent/exports/`**: Atlas/HTML/UI exports and graph/metrics helpers.
- **`pmagent/mcp/`**: Postgres-backed MCP catalog/knowledge integration.
- **`pmagent/tests/`**: Test suites for the above components.
...
**Implementation:**
- **Unified helper**: `pmagent.status.snapshot.get_system_snapshot()` — Single source of truth for system snapshot composition
...
- **Eval exports helpers**: `pmagent.status.eval_exports` — Shared helpers for reading Phase-8/10 eval export files (hermetic, tolerant of missing files)
...
- pmagent CLI subcommands (e.g. `pmagent health *`, `pmagent status *` including `pmagent status kb` for KB registry status view, `pmagent reality-check check`) are thin wrappers over `pmagent.*` modules.
...
- Primary tests live under `pmagent/tests/` and are invoked via:

  ```bash
  pytest -q pmagent/tests
  ```

- Additional end-to-end coverage comes from:
  - `pmagent` CLI tests (e.g. `pmagent/tests/cli/`)
  - Atlas/UI tests (e.g. `pmagent/tests/atlas/`, `tests/ui/*`)
  - Guard, MCP, and system status tests.

## Development Guidelines

- **DSN & Postgres**
  - Never hard-code DSNs; always use `scripts.config.env` or `pmagent.db.loader`.
...
- **AI tracking**
  - All LM calls should be logged through `pmagent.runtime.lm_logging` helpers when DB is available:
  ...
  - All pmagent CLI commands should use `pmagent.control_plane.create_agent_run()` and `mark_agent_run_success()` / `mark_agent_run_error()` to track command executions in `control.agent_run_cli`
