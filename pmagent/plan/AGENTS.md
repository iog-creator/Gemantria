...
The `pmagent/plan/` directory contains planning workflows powered by the KB registry. These helpers provide deterministic planning surfaces for PM/AgentPM workflows, including both read-only worklist generation and doc-fix execution.
...
**Implementation:**
- **Helper**: `pmagent.plan.kb.build_kb_doc_worklist()` — Builds prioritized worklist from KB registry status and hints
...
- `subsystem`: Owning subsystem (e.g., "docs", "pmagent", "webui")
...
**Integration:**
- Uses `pmagent.status.snapshot.get_kb_status_view()` and `get_kb_hints()` for KB data
- Uses `pmagent.kb.registry.load_registry()` for document details
...
**Implementation:**
- **Helper**: `pmagent.plan.fix.build_fix_actions()` — Converts worklist items to fix action objects
- **Helper**: `pmagent.plan.fix.apply_actions()` — Executes fix actions (dry-run or apply mode)
...
- Uses `pmagent.kb.registry` for document metadata and freshness tracking
...
**Implementation:**
- **Helper**: `pmagent.plan.next.build_next_plan()` — Reads `docs/SSOT/MASTER_PLAN.md` and `NEXT_STEPS.md`, extracts `Current Focus`, `Next Milestone`, and the latest "Next Gate / Next Steps" bullets.
...
**Implementation:**
- **Helper**: `pmagent.plan.next.build_capability_session()` — Builds a capability_session envelope for a specific candidate ID
...
**Implementation:**
- **Helper**: `pmagent.plan.next.run_reality_loop()` — Runs a single planning + posture loop and persists capability_session envelope
...
**Implementation:**
- **Helper**: `pmagent.plan.next.list_capability_sessions()` — Reads and summarizes capability_session envelopes
