...
class RerankerAdapter:
    ...
    def _ensure_lm(self) -> bool:
        ...
        """
        from pmagent.adapters.lm_studio import get_lm_model_config
        ...
        try:
            from pmagent.adapters.lm_studio import rerank
    ...
    def rerank_chunks(self, chunks: list[dict[str, Any]], query: str) -> list[dict[str, Any]]:
        ...
        Rule 045 blend: edge_strength = EDGE_ALPHA * cosine + (1 - EDGE_ALPHA) * rerank_score
        Default: EDGE_ALPHA = 0.5 (equal weight)

        Args:
            chunks: Initial chunks from embedding-based retrieval.
                    Each chunk must have 'cosine' field (embedding similarity score).
            query: Original query string

        Returns:
            Reranked chunks with updated reranker_score and edge_strength fields,
            sorted by edge_strength (descending).

        Raises:
            RuntimeError: If LM unavailable (Wave-3 requires LM).
        """
        if not chunks:
            return []
        ...
        for chunk in chunks:
            ...
            chunk["rerank_score"] = reranker_score

        # Compute edge_strength using Rule 045 blend for each chunk
        for chunk in chunks:
        ...
        sorted_chunks = sorted(chunks, key=lambda c: c.get("edge_strength", 0.0), reverse=True)
