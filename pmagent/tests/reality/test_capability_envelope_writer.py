...
from pmagent.control_plane import AgentRun
from pmagent.reality.capability_envelope_writer import maybe_persist_capability_session
from pmagent.reality.capability_envelope_validator import validate_and_optionally_persist
...
def test_maybe_persist_capability_session_db_off() -> None:
    ...
    with patch("pmagent.reality.capability_envelope_writer.get_rw_dsn", return_value=None):
...
def test_maybe_persist_capability_session_db_on_happy_path() -> None:
    ...
    with patch("pmagent.reality.capability_envelope_writer.get_rw_dsn", return_value="postgresql://test"):
        with patch("pmagent.reality.capability_envelope_writer.create_agent_run", mock_create_agent_run):
            result = maybe_persist_capability_session(envelope, tracking_enabled=True)
...
def test_maybe_persist_capability_session_handles_create_agent_run_none() -> None:
    ...
    with patch("pmagent.reality.capability_envelope_writer.get_rw_dsn", return_value="postgresql://test"):
        with patch("pmagent.reality.capability_envelope_writer.create_agent_run", return_value=None):
            result = maybe_persist_capability_session(envelope, tracking_enabled=True)
...
def test_maybe_persist_capability_session_handles_exceptions() -> None:
    ...
    with patch("pmagent.reality.capability_envelope_writer.get_rw_dsn", return_value="postgresql://test"):
        with patch(
            "pmagent.reality.capability_envelope_writer.create_agent_run",
