...
def classify_fragment(text: str, doc_path: str) -> dict[str, Any]:
    ...
    """
    system_prompt = """Your sole function is structured data extraction and classification.
You are absolutely forbidden from generating interpretation or narrative text.
Output must conform exactly to the classification schema.
...
Return JSON with: subsystem, doc_role, importance, phase_relevance, should_archive, kb_candidate."""
    ...
    try:
        ...
        # Log successful classification to control.agent_run
        _write_agent_run(
            tool="kb_classify_fragment",
            args_json={"doc_path": doc_path, "text_length": len(text)},
            result_json={"meta": result, "ok": bool(result)},
        ...
        # Log classification failure to control.agent_run
