# AGENTS.md - pmagent/reality Directory

## Directory Purpose

The `pmagent/reality/` directory contains the reality check system for comprehensive environment validation (env/DSN, DB/control plane, LM/models, exports, eval smokes).
...
**Location:** `pmagent/reality/check.py`
...
Validates LM health and model slot status using `scripts/guards/guard_lm_health.py` and `pmagent/lm/lm_status.py`.
...
**Location:** `pmagent/reality/capability_envelope_validator.py`
...
**Location:** `pmagent/reality/capability_envelope_writer.py`
...
**DB Integration:**
- Uses `pmagent.control_plane.create_agent_run()` helper
...
- `pmagent/control_plane.py` - `create_agent_run()` helper
...
**Location:** `pmagent/reality/sessions_summary.py`
